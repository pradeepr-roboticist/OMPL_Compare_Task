...
# Retrieve Data form the database (set the local path to the scenarios.db)
connect = sqlite3.connect('/home/lamy/Desktop/OMPL_Compare_Task/VREP_Test_Maps/scenarios.db')
cursor = connect.cursor()
cursor.execute('SELECT scenario_id FROM scenarios')
scenario_id = max(cursor.fetchall())
scenario_id = int(scenario_id[0])
start = [[] for _ in range(scenario_id)]
goal = [[] for _ in range(scenario_id)]
Map = [[] for _ in range(scenario_id)]
for i in range(0, scenario_id, 1):
    cursor.execute('SELECT start_state FROM scenarios WHERE scenario_id = '+str(i))
    startdb = cursor.fetchall()
    startdb = listToStringWithoutBrackets(startdb)
    startdb = startdb.replace(',','').replace('(',"").replace(')',"").replace("'","")
    startdb = list(map(float,startdb.split()))
    cursor.execute('SELECT goal_state FROM scenarios WHERE scenario_id = '+str(i))
    goaldb = cursor.fetchall()
    goaldb = listToStringWithoutBrackets(goaldb)
    goaldb = goaldb.replace(',','').replace('(',"").replace(')',"").replace("'","")
    goaldb = list(map(float,goaldb.split()))
    cursor.execute('SELECT map_name FROM scenarios WHERE scenario_id = '+str(i))
    VrepMap = cursor.fetchall()
    VrepMap = listToStringWithoutBrackets(VrepMap)
    VrepMap = VrepMap.replace(',','').replace('(',"").replace(')',"").replace("'","")
    VrepMap = VrepMap + '.stl'
